...
const PASSWORD_CREDENTIAL_TYPES = {
  SHA256: 'sha256',
  LEGACY_MD5: 'legacy-md5'
}

const formatPasswordCredential = (type, digest) => `${type}:${digest}`

const parsePasswordCredential = value => {
  if (typeof value !== 'string') return null
  const [type, digest] = value.split(':')
  if (!digest || !Object.values(PASSWORD_CREDENTIAL_TYPES).includes(type)) {
    return null
  }
  return { type, digest }
}

export const isPasswordCredential = value =>
  Boolean(parsePasswordCredential(value))

export const getPasswordCredentialForPost = (post, passInput) => {
  if (!post?.password || !passInput) return null

  const storedCredential = parsePasswordCredential(passInput)
  if (storedCredential?.digest === post.password) {
    return passInput
  }

  const nextHash = sha256Digest(passInput)
  if (nextHash === post.password) {
    return formatPasswordCredential(
      PASSWORD_CREDENTIAL_TYPES.SHA256,
      post.password
    )
  }

  const legacy = md5(String(post?.slug ?? '') + passInput)
  if (legacy === post.password) {
    return formatPasswordCredential(
      PASSWORD_CREDENTIAL_TYPES.LEGACY_MD5,
      post.password
    )
  }
